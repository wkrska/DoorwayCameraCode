...
function test_sparsity_posNeg_constrain(renderNum, bgStr, frameStr,lambdaGroup, dirpath,nIter, stepSize, plotFreq)
%% Init
close all

global meas_pix
% addpath('/Users/sheilawerth/Dropbox/Lab/two corner camera/ICCP2022/realResults/');
% switch(num_obj)
%     case 1
%         filepath = 'oneTarget/';
%     case 2
%         filepath = 'twoTargets/data/';
%     case 3
%         filepath = 'threeTargets/';
% end

% renderNum = 0; % which data to use
seed = 0;
weight = 0;
ambMult = 0;
rmMult = 0;
...
rec_size = [45,45]; % Number of 'room pixels' to recover
...
tic
% filename = strcat(num2str(meas_pix),'p-',num2str(rec_size(1)),'r-',num2str(round(meas_size*100)),'cm-min-pi_',num2str(pi/p.min_angle),'.mat');
...
try
    ...
    tic
    [A, recon_grid, S] = makeA3D_cylindrical_plane_modified_1m(pos,1,16,p,1);
    toc
...
fg_range = room_dim(1)*.5; % everything inside of this range must be positive
...
ifiltx = find(x_coord<room_dim(1)*1/3);
ifilty = find(y_coord<room_dim(1)*2/3);
...
% figure
% imagecyl(testScene./(max(max(testScene))),recon_grid);
% axis square; set(gca,'color','black');
% % xlim([0,room_dim]);
% % ylim([0,room_dim]);
% axis on;
% xlabel('x [m]','Interpreter','latex','FontSize',14)
% ylabel('y [m]','Interpreter','latex','FontSize',14)
% scatter(pos(:,1),pos(:,2),'filled','y')
% xlim([0 3])
% ylim([0 3])
% 
% fig = gcf;
...
% fig.Units = 'centimeters';
% print(fig,['graphPaperOurGrid.pdf'],'-dpdf','-r200');

%% Load Measurements, resize to 256x256
disp("Loading measurements");
tic

% num_exp_per_scn = 1; %Number of exposures per scene
resolution = 1024;
bg1 = zeros(resolution);
bg2 = zeros(resolution);
newFrame1 = zeros(resolution);
newFrame2 = zeros(resolution);
num_exp_per_scn = 1;
for exp = 0:num_exp_per_scn-1
    bg1 = bg1+double(imread([bgStr '_left.jpg']))/256/num_exp_per_scn;
    bg2 = bg2+double(imread([bgStr '_right.jpg']))/256/num_exp_per_scn;
    newFrame1 = newFrame1+double(imread([frameStr '_left.jpg']))/256/num_exp_per_scn;
    newFrame2 = newFrame2+double(imread([frameStr '_right.jpg']))/256/num_exp_per_scn;
end

% newFrame1 = double(imread(strcat('frame_',num2str(renderNum),'_samps_',num2str(samps),'_left_meas.png')))/2^16;
% newFrame2 = double(imread(strcat('frame_',num2str(renderNum),'_samps_',num2str(samps),'_right_meas.png')))/2^16;
% bg1 = double(imread(strcat('frame_5_samps_',num2str(samps),'_left_meas.png')))/2^16;
% bg2 = double(imread(strcat('frame_5_samps_',num2str(samps),'_right_meas.png')))/2^16;
...
newFrame1 = imresize(newFrame1(cropX1, cropY1, :),[meas_pix,meas_pix],"Antialiasing",true);
newFrame2 = imresize(newFrame2(cropX1, cropY1, :),[meas_pix,meas_pix],"Antialiasing",true);

bg1 = imresize(bg1(cropX1, cropY1, :),[meas_pix,meas_pix],"Antialiasing",true);
bg2 = imresize(bg2(cropX1, cropY1, :),[meas_pix,meas_pix],"Antialiasing",true);
...
% 
% subplot(1,3,2)
% imagesc(newFrame1(:,:,1)-newFrame1(:,:,2))
% title('R new frame')
% colorbar; axis off;
% 
% 
% subplot(1,3,3)
% imagesc(meas1(:,:,1)-meas1(:,:,2))
% title('R diff')
% colorbar; axis off;

% meas1plot = meas1;
% meas1plot(meas1plot<0) = 0;
% 
% 
% meas2plot = meas2;
% meas2plot(meas2plot<0) = 0;
...
fig.PaperUnits = 'centimeters';
fig.Units = 'centimeters';
fig.PaperPosition = [-.388 -.538 1 1];
fig.PaperSize=[.226 .226];
...
saveStuff = 1;
debias = 0;
toggles = [plotStuff,saveStuff,debias,weight];
S = sparse(S);
tic
...
%             filename = strcat('grp-',num2str(lambdaGroup), '-scn-',num2str(renderNum),'.mat');
            filename = strcat('real-data-grp-',num2str(lambdaGroup), '-scn-',num2str(renderNum), '-frame-',num2str(renderNum),'.mat');
